...
public class ApiSwaggerService{
	private @Autowired Swagger2 swaggerProcess;
	    ...
	    for (ProjectManage pm : list) {
			...
			for (SwaggerInfo swg :listUrl ) {
				...
				if(swg.getUrl().contains("swagger-resources")) {
					String baseurl=swg.getUrl().split("/swagger-resources")[0];
					...
					if(JSONUtil.isJson(jaStr)) {
						...
						for (int i = 0; i < array.size(); i++) {
							JSONObject resource=JSONUtil.parseObj(array.get(i));
							swg.setUrl(baseurl+resource.getStr("url"));
							swg.setServiceName(resource.getStr("name"));
	...
	public Boolean updateSwaggerAPI(Integer pid,SwaggerInfo swg) {
		...
		for (int i=0;i<apiSwaggerList.size();i++) {
			...
			queryWrapper.lambda().eq(ApiSwagger::getApiUri, actual.getApiUri())
								 .eq(ApiSwagger::getApiMethod, actual.getApiMethod())
								 .eq(ApiSwagger::getServiceName, actual.getServiceName());;
